...
static inline bool IsTAAUpsamplingConfig(ETAAPassConfig Pass)
{
	return Pass == ETAAPassConfig::MainUpsampling || Pass == ETAAPassConfig::DiaphragmDOFUpsampling;
}

static inline bool IsMainTAAConfig(ETAAPassConfig Pass)
{
	return Pass == ETAAPassConfig::Main || Pass == ETAAPassConfig::MainUpsampling;
}

static inline bool IsDOFTAAConfig(ETAAPassConfig Pass)
{
...
struct FTAAPassParameters
{
	...
	void SetupViewRect(const FViewInfo& View, int32 InResolutionDivisor = 1)
	{
