...
void FSystemTextures::InternalInitializeTextures()
{
	...
	if (!bTexturesInitialized)
	{
		...
		{
			PooledRenderTargetDesc Desc(PooledRenderTargetDesc::Create2DDesc(FIntPoint(1, 1), PF_B8G8R8A8, FClearValueBinding::White, TexCreate_HideInVisualizeTexture, TexCreate_RenderTargetable | TexCreate_NoFastClear, false));
			...
			RHICopyToResolveTarget(WhiteDummy->TargetableTexture.get(), WhiteDummy->ShaderResourceTexture.get(),FResolveParams());
		}

		// Create a BlackDummy texture
		{
			PooledRenderTargetDesc Desc(PooledRenderTargetDesc::Create2DDesc(FIntPoint(1, 1), PF_B8G8R8A8, FClearValueBinding::Transparent, TexCreate_HideInVisualizeTexture, TexCreate_RenderTargetable | TexCreate_NoFastClear, false));
			Desc.AutoWritable = false;
			...
			RHICopyToResolveTarget(BlackDummy->TargetableTexture.get(), BlackDummy->ShaderResourceTexture.get(), FResolveParams());
		}

		// Create a BlackAlphaOneDummy texture
		{
			PooledRenderTargetDesc Desc(PooledRenderTargetDesc::Create2DDesc(FIntPoint(1, 1), PF_B8G8R8A8, FClearValueBinding::Black, TexCreate_HideInVisualizeTexture, TexCreate_RenderTargetable | TexCreate_NoFastClear, false));
			Desc.AutoWritable = false;
			...
			RHICopyToResolveTarget(BlackAlphaOneDummy->TargetableTexture.get(), BlackAlphaOneDummy->ShaderResourceTexture.get(), FResolveParams());
		}
		...
		// Create a GreenDummy texture
		{
			...
			RHICopyToResolveTarget(GreenDummy->TargetableTexture.get(), GreenDummy->ShaderResourceTexture.get(), FResolveParams());
		}
		...
		// Create a DefaultNormal8Bit texture
		{
			...
			RHICopyToResolveTarget(DefaultNormal8Bit->TargetableTexture.get(), DefaultNormal8Bit->ShaderResourceTexture.get(), FResolveParams());
		}
		// Create the PerlinNoiseGradient texture
		{
			PooledRenderTargetDesc Desc(PooledRenderTargetDesc::Create2DDesc(FIntPoint(128, 128), PF_B8G8R8A8, FClearValueBinding::None, TexCreate_HideInVisualizeTexture, TexCreate_None | TexCreate_NoFastClear, false));
			Desc.AutoWritable = false;
		...
		{
			PooledRenderTargetDesc Desc(PooledRenderTargetDesc::Create2DDesc(FIntPoint(32, 16), PF_R16_UINT, FClearValueBinding::None, TexCreate_HideInVisualizeTexture, TexCreate_NoFastClear, false));
			Desc.AutoWritable = false;
		...
		{
			PooledRenderTargetDesc Desc(PooledRenderTargetDesc::Create2DDesc(FIntPoint(1, 1), PF_FloatRGBA, FClearValueBinding(FLinearColor(65000.0f, 65000.0f, 65000.0f, 65000.0f)), TexCreate_HideInVisualizeTexture, TexCreate_RenderTargetable | TexCreate_NoFastClear, false));
			Desc.AutoWritable = false;
			GRenderTargetPool.FindFreeElement(Desc, MaxFP16Depth, TEXT("MaxFP16Depth"));

			FRHIRenderTargetView RtView = FRHIRenderTargetView(MaxFP16Depth->TargetableTexture, ERenderTargetLoadAction::EClear);
		...
		// Create dummy 1x1 depth texture		
		{
			...
			RHICopyToResolveTarget(DepthDummy->TargetableTexture.get(), DepthDummy->ShaderResourceTexture.get(), FResolveParams());
		}
	}
#if 0
	...
	//if (InFeatureLevel >= ERHIFeatureLevel::SM4)
	{
		...
		PooledRenderTargetDesc Desc(PooledRenderTargetDesc::CreateVolumeDesc(Extent, Extent, Extent, PF_B8G8R8A8, FClearValueBinding::None, TexCreate_ShaderResource | TexCreate_HideInVisualizeTexture | TexCreate_NoTiling, TexCreate_None, false));
		Desc.AutoWritable = false;
		GRenderTargetPool.FindFreeElement(RHICmdList, Desc, PerlinNoise3D, TEXT("PerlinNoise3D"), true);
		// Write the contents of the texture.
		...
		RHICmdList.UpdateTexture3D(
			(FTexture3DRHIRef&)PerlinNoise3D->ShaderResourceTexture,
			0,
			Region,
	...
	//if (InFeatureLevel >= ERHIFeatureLevel::SM4)
	{
...

		{
			...
			PooledRenderTargetDesc Desc(PooledRenderTargetDesc::Create2DDesc(FIntPoint(128, 32), Format, FClearValueBinding::None, 0, TexCreate_None, false));
			Desc.AutoWritable = false;
			if (bReference)
			{
				Desc.Extent.X = 128;
