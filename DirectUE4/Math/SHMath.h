...
public:
	...
	static constexpr double ConstantBasisIntegral = 3.5449077018110320545963349666823; // 2 * Sqrt(PI)
	...
	friend inline float Dot(const TSHVector& A, const TSHVector& B)
	{
...
public:
	...
	friend inline TSHVectorRGB operator*(const TSHVectorRGB& A, const FLinearColor& Color)
	{
		TSHVectorRGB Result;
	...
	/** Color multiplication operator. */
	friend inline TSHVectorRGB operator*(const FLinearColor& Color, const TSHVectorRGB& A)
	{
		TSHVectorRGB Result;
		Result.R = A.R * Color.R;
		Result.G = A.G * Color.G;
	...
	/** Dot product operator. */
	friend inline FLinearColor Dot(const TSHVectorRGB& A, const TSHVector<MaxSHOrder>& InB)
	{
		FLinearColor Result;
	...
	inline void AddIncomingRadiance(const FLinearColor& IncomingRadiance, float Weight, const Vector4& WorldSpaceDirection)
	{
...
/** Color multiplication operator. */
template<int32 Order>
inline TSHVectorRGB<Order> operator*(const TSHVector<Order>& A, const FLinearColor& B)
{
