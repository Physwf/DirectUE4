...
{
	int32 NumToAdd = Planes.size() / 4;
	int32 NumRemaining = Planes.size() % 4;
	// Presize the array
...
static inline bool IntersectBoxWithPermutedPlanes(
	...
	const VectorRegister BoxExtent)
{
	...
	assert(PermutedPlanes.size() % 4 == 0);
	...
	const FPlane* PermutedPlanePtr = (FPlane*)PermutedPlanes.data();
	// Process four planes at a time until we have < 4 left
	for (uint32 Count = 0, Num = PermutedPlanes.size(); Count < Num; Count += 4)
	{
...
bool FConvexVolume::IntersectBox(const FVector& Origin, const FVector& Extent, bool& bOutFullyContained) const
{
	...
	const FPlane* PermutedPlanePtr = (FPlane*)PermutedPlanes.data();
	// Process four planes at a time until we have < 4 left
	for (uint32 Count = 0; Count < PermutedPlanes.size(); Count += 4)
	{
		// Load 4 planes that are already all Xs, Ys, ...
		VectorRegister PlanesX = VectorLoadAligned(PermutedPlanePtr);
		PermutedPlanePtr++;
